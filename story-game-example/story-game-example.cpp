...
struct Scene {
        ...
        std::unordered_map<char, int>
            choices;  // Map: input character -> next scene ID
};

// The game class handles the game logic and state
class Game {
       private:
        std::unordered_map<int, Scene>
            scenes;        // Collection of all scenes indexed by scene ID
        ...
        void displayScene() {
                ...
                for (const auto& choice : scene.choices) {
                        // choice.first = input key (e.g. 'L'), choice.second = next scene ID
                        // (e.g. 2)
        ...
        // Function to get player's input and move to the next scene based on the
        // input
        void processInput() {
       ...
       public:
        ...
        Game() : currentScene(1) {
                // Define each scene by assigning a Scene struct to the scenes map using an
                // ID
                ...
                scenes[2] = Scene{
                    ...
                    {{'O', 4},
                     {'B', 1}}  // 'O' opens chest -> scene 4, 'B' goes back -> scene 1
                };
                ...
                scenes[4] = Scene{
                    "You found gold! You win.\nGame Over.", {}  // No choices, game ends here
                };
