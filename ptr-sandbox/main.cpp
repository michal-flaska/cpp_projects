...
void section(const char* t) { std::cout << "\n=== " << t << " ===\n"; }
...
int main() {
        ...
        *m = 77;
        ...
        std::cout << std::setw(6) << "i" << std::setw(6) << "val" << std::setw(10)
                  << "offset"
                  << "addr\n";

        for (int i = 0; i < 5; i++) {
                std::uintptr_t addr = reinterpret_cast<std::uintptr_t>(&arr[i]);
                std::cout << std::setw(6) << i << std::setw(6) << arr[i] << std::setw(10)
                          << (addr - base) << &arr[i] << '\n';
        }
        ...
        section("vtable peek");
        Derived d;
        std::uintptr_t* vptr = *reinterpret_cast<std::uintptr_t**>(&d);
        kv("&object", &d);
        kvu("vtable ptr", *vptr);
        ...
        int y = 1;
        int* bad = reinterpret_cast<int*>(reinterpret_cast<char*>(&y) + 1);
        *bad = 999;
        kvi("y after UB", y);
#endif
