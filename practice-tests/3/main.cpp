...
// If there's no second largest (all numbers same, or only one number), return -1.
...
int secondLargest(std::vector<int> nums){
    if(nums.size() < 2) return -1;  // Not enough numbers
    ...
    for(int i = 0; i < nums.size(); i++){
        if(nums[i] < largest){                          // Must be smaller than largest
            if(second == -1 || nums[i] > second){       // If it's bigger than current second
                second = nums[i];
            }
        }
    }
