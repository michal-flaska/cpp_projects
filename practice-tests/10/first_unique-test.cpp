...
TEST(FirstUnique, BasicTest) {
  EXPECT_EQ('h', firstUnique("hello"));
}


class FirstUniqueTest : public ::testing::TestWithParam<std::pair<std::string, char>> {
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(      // List of parameters (input, expected)
        std::make_pair("hello", 'h'),
        std::make_pair("leetcode", 'l'),
